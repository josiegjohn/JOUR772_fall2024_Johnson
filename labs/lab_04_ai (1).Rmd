...
```{r}
library(axolotr)
create_credentials(GROQ_API_KEY = "my key here")
```
...
# use paste to combine prompt with 
...
evans_response <- axolotr::ask(
  prompt = paste("Given the following text, extract information into a CSV file data table with these 6 variables: title, year, role, directors, co-stars ,awards" , text),
  model = "llama-3.1-8b-instant"
)
...
**Answer: Groq did not add the titles I gave it as titles at first. It seems very confused on what is a movie or TV show title, but it did include the titles that were explicitly said in the article. Every run is pretty different, which is similar to how ChatGPT functions so that doesn't surprise me. It doesn't seem to know how to search the internet for the other Harry Potter movies and only lists the two that were spelled out in the article, even though the article says that she was in more than that.**
...
idea_response <- axolotr::ask(
  prompt = "what does this code do with the table umd_courses, please explain to me what each line of code does", r_code_example,
  model = "llama-3.1-8b-instant"
)
...
**Answer: I believe that the answer, by line of code, is correct.**
...
```{r}
#Loading lab question 4 on lab 2
...
#Add code below to talk to Groq and display its response
idea_response <- axolotr::ask(
  prompt = "is this code correct" , your_r_code_lab
...
**Answer: It's kind of silly, because it's saying that having unknown states be "na" may not be my desired result, but it seems to understand what I had for the code. It gave me a sample of code that wants to list unknown states as "DC" which could be an issue.**
...
```{r}
#Add code below to talk to Groq and display its response in the text describe the data set or download data set as csv 
idea_response <- axolotr::ask(
  prompt = "please share ideas about potential news stories from this data set on Montgomery County Maryland crash reports. The data set includes names of roads, hit and runs, and more" 
...
**Answer: I didn't describe the data set in too much detail, so Groq's responses have some duds but is overall very helpful. Obviously I would examine the data myself to see if anything Groq suggests is actually taking place, but some of the ideas are solid.**
...
#Add code below to talk to Groq and display its response
idea_response <- axolotr::ask(
  prompt = "Please write a 500-word news article about a car crash at the intersection between Knox Road and Regents Drive"
,
  model = "llama-3.1-8b-instant"
)
...
**Answer: For the sake of a simple answer, we're going to ignore the fact that if this was a real story, the AI could very likely be making this stuff up.**

The format is pretty good, and I'm impressed that it recognizes the name of the driver differently than the names of the other victims, who it says are not to be mentioned. The writing is not very clear and repeats itself but honestly it's better than I expected.

And to be picky, there's some pretty glaring AP style issues.
