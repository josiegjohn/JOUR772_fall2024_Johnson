...
overdose_combined <- bind_rows(list(cecil, carroll, allegany))
...
baltimore <- read_csv("data/baltimore_911.csv") |>
  clean_names()
zcta <- read_csv("data/maryland_zcta.csv") |>
  clean_names() |>
  rename(zip_code=zcta5n)


baltimore_zipcode <- baltimore |>
  left_join(zcta, join_by(zip_code))
```
...
A2. Grove neck Rd has the most with 15
...
baltimore_zipcode |>
  ...
  summarize(total=n()) |>
  mutate(ihatethis = total/population * 1000)
...
Q4: In Neighborhoods with a high population, so 15 or more percent, of people over 65: which has the most calls?

A4: Woodmere has the most calls with a total of 66. And their percentage of the population over 65 years old is 16.8 percent.

```{r}
baltimore_zipcode |>
  filter(pct_pop_65 > 15) |>
  arrange(desc(pct_pop_65)) |>
  group_by(neighborhood, pct_pop_65) |>
  summarize(total=n()) |>
  arrange(desc(total))
```
