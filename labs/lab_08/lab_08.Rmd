...
```{r}
#calculate percentage for every county
md_absentee_ballots <- md_absentee_ballots |> mutate(pct_returned = (total_received / total_sent)*100)
#we need top 10 counties now
top10_ballots <- md_absentee_ballots |> arrange(desc(pct_returned)) |>
  head(10)
#now we need chart
top10_ballots |>
  ggplot() +
  ...
  coord_flip() +
  labs(
    title="Top 10 Counties with Returned Absentee Ballots",
    x = "County",
...
**A2. Saint Mary's has the most returns.**
...
#top10_ballots_rep <- md_absentee_ballots |>
  #mutate(pct_returned_rep = (rep_received/total_received)*100) |>
  #arrange(pct_returned_rep) |> head(10) 
  
#top10_ballots_demrep |>
  #mutate(pct_diff = pct_returned_dem - pct_returned_rep)

#top10_ballots_demrep <- top10_ballots_dem |>
  #bind_rows(top10_ballots_rep) |> head(10)
...
md_absentee_ballots |>
  ggplot() +
  geom_bar(aes(x=county_name, weight=pct_diff))  +
  coord_flip() +
  labs(
    title="Top 10 Counties with Returned Absentee Ballots",
    x = "Counties",
...
**A3 The chart is telling us that Democrat's ballots are being returned at a much higher rate. From what I understand, this makes sense. I assume there is a higher percentage of Democrats voting absentee vs Republicans. More are returned as time passes, which makes sense assuming more are being sent closer to the election.**
...
frederick_county |>
 ...
 ggplot(mpg, aes(displ, hwy)) +
  geom_line(aes(x=ballots_returned, y= ballots_received)) +
  geom_point() +
 ...
 labs(
    ...
    caption = "source: Maryland State Board of Elections")
 geom_text()
# I could not for the life of me figure this one out. I wasn't working alone, out of that whole group of us that stays after to work on labs none of us could figure it out. (Maybe they have since then, but I haven't.) 
```
