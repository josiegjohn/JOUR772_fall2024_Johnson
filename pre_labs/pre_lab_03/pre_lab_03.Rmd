...
#r decided that vendor zip is character column, so after 10 rows there is a character somewhere in that vendor zip column
```
...
#were looking for how many weird zip codes, filter and count how many of which
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer n/a in 2010 and 2011 when each has 1 payment**
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer a grant for \$60 is kind of weird**
...
#makes column names better, always clean them up 
...
#we renamed grantor to source, to make it make more sense
```
...
**Task** Run the following codeblock to use str_to_upper() to make the source, grantee and description columns upper-cased. (so that the filter can read it all the same)
...
# distinct is what removes dupes(if youre sure they should be removed)
```
...
# didnt replace the zip code, but created a new column named zip5
